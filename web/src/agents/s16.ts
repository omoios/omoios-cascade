...
type MergeStatus = "Applied" | "Conflict" | "NoChanges";
...
const PLANNER_SYSTEM = [
  ...
  "Workers run in private workspaces.",
  "When handoffs are ready, run optimistic_merge.",
  "DO NOT REVERT. On conflict, create fix-forward task.",
].join("\n");
...
export class OptimisticMergeAgent extends BaseAgent {
  ...
  private fixTasks: FixForwardTask[] = [];
  private mergeLogs: MergeLogEntry[] = [];
  ...
  private seedPlannerScratchpad(): void {
    ...
    this.toolExecutor.fs.writeFile(
      ...
      [
        ...
        "## Role Constraint",
        "- I NEVER write code.",
        "- I decompose, delegate, and merge handoffs.",
        "",
        "## Merge Rule",
        "- Use file-based optimistic 3-way merge.",
  ...
  private createRuntime(taskId: string, workspacePath: string, baseSnapshot: Record<string, string>): WorkerRuntime {
    return {
      taskId,
      startedAt: Date.now(),
      attempts: 0,
  ...
  private seedWorkerScratchpad(worker: string, task: string, workspacePath: string): void {
    this.rewriteWorkerScratchpad(
      ...
      [
        ...
        "- Handoff diff is against base snapshot from spawn.",
        "",
        "## Assigned Task",
        `- ${task}`,
      ].join("\n")
  ...
  private async composeNarrative(worker: string, handoff: Handoff): Promise<string> {
    ...
    return [
      ...
      "Next step: planner runs optimistic_merge for this handoff.",
    ].join("\n");
  }
  ...
  private async spawnWorker(input: Record<string, unknown>): Promise<string> {
    ...
    this.runtimes.set(name, this.createRuntime(taskId, workspace.workspacePath, baseSnapshot));
  ...
  private findHandoff(input: Record<string, unknown>): Handoff | undefined {
    const handoffId = input.handoff_id ? String(input.handoff_id) : undefined;
    const taskId = input.task_id ? String(input.task_id) : undefined;
    const agentId = input.agent_id ? String(input.agent_id) : undefined;

    const selected = this.handoffs.filter((h) => {
      if (handoffId && h.handoff_id !== handoffId) return false;
      if (taskId && h.task_id !== taskId) return false;
      if (agentId && h.agent_id !== agentId) return false;
      return true;
    });
  ...
  private threeWayMerge(base: Record<string, string>, ours: Record<string, string>, theirs: Record<string, string>) {
    const paths = Array.from(new Set([...Object.keys(base), ...Object.keys(ours), ...Object.keys(theirs)])).sort();

    const mergedChanges: Record<string, string | null> = {};
    const conflicts: Record<string, { base: string; ours: string; theirs: string }> = {};
    const unchanged: string[] = [];

    for (const path of paths) {
      const baseValue = base[path] ?? null;
      const ourValue = ours[path] ?? null;
      const theirValue = theirs[path] ?? null;
    ...
    return { mergedChanges, conflicts, unchanged, attempted: paths.length };
  }

  private applyChangesToCanonical(changes: Record<string, string | null>): string[] {
    const applied: string[] = [];
    for (const [path, content] of Object.entries(changes)) {
      if (content === null) {
        if (this.toolExecutor.fs.getFile(path) !== undefined) {
          this.toolExecutor.fs.writeFile(path, "");
        }
      } else {
        this.toolExecutor.fs.writeFile(path, content);
      }
      applied.push(path);
    }
    return applied.sort();
  ...
  private createFixForwardTask(
    handoff: Handoff,
    conflicts: Record<string, { base: string; ours: string; theirs: string }>
  ): FixForwardTask {
    const task: FixForwardTask = {
      fix_task_id: `fix-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      source_handoff_id: handoff.handoff_id,
      source_agent_id: handoff.agent_id,
      source_task_id: handoff.task_id,
      title: `Resolve merge conflicts for ${handoff.agent_id}/${handoff.task_id}`,
      description:
        "Fix-forward required. Merge conflict detected during optimistic merge. DO NOT REVERT. The repo may be broken. This is by design.",
      conflicts,
      status: "pending",
      created_at: Date.now(),
    };

    this.fixTasks.push(task);
    this.appendInbox(this.plannerName, {
      type: "message",
      from: this.plannerName,
      to: this.plannerName,
      content: `Fix-forward task created: ${task.fix_task_id}`,
      fix_task: task,
      timestamp: Date.now(),
    });
    return task;
  ...
  private reconstructOursFromHandoff(base: Record<string, string>, handoff: Handoff): Record<string, string> {
    const ours = { ...base };
    for (const [path, delta] of Object.entries(handoff.diff)) {
      ours[path] = delta.after;
    }
    return ours;
  }

  private optimisticMerge(input: Record<string, unknown>): string {
    const handoff = this.findHandoff(input);
    if (!handoff) return "Error: handoff not found";
    if (handoff.merged) return `Handoff already merged (${handoff.merge_status ?? "unknown"})`;

    const rt = this.runtimes.get(handoff.agent_id);
    if (!rt) return `Error: runtime missing for ${handoff.agent_id}`;

    const base = rt.baseSnapshot;
    const ours = this.reconstructOursFromHandoff(base, handoff);
    const theirs = this.toolExecutor.fs.snapshot();

    const result = this.threeWayMerge(base, ours, theirs);
    const appliedFiles = this.applyChangesToCanonical(result.mergedChanges);
    const conflictedFiles = Object.keys(result.conflicts).sort();

    let status: MergeStatus = "NoChanges";
    let note = "No file-level worker changes to apply.";
    let fixTaskId: string | null = null;

    if (conflictedFiles.length > 0) {
      status = "Conflict";
      const fixTask = this.createFixForwardTask(handoff, result.conflicts);
      fixTaskId = fixTask.fix_task_id;
      note =
        "Conflict detected. Applied non-conflicting files and created fix-forward task. No revert performed.";
    } else if (appliedFiles.length > 0) {
      status = "Applied";
      note = "Clean optimistic merge applied.";
    }

    const mergeLog: MergeLogEntry = {
      log_id: `m-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      timestamp: Date.now(),
      handoff_id: handoff.handoff_id,
      worker: handoff.agent_id,
      task_id: handoff.task_id,
      status,
      attempted_files: result.attempted,
    ...
    this.mergeLogs.push(mergeLog);
    handoff.merged = true;
    handoff.merge_status = status;
    handoff.merge_log_id = mergeLog.log_id;
  ...
  private listFixTasks(input: Record<string, unknown>): string {
    const onlyPending = Boolean(input.only_pending);
    const selected = this.fixTasks.filter((t) => !onlyPending || t.status === "pending");
    return JSON.stringify(selected, null, 2);
  }

  private readMergeLog(input: Record<string, unknown>): string {
    const rawLimit = Number(input.limit ?? 20);
    const limit = Number.isFinite(rawLimit) && rawLimit > 0 ? Math.floor(rawLimit) : 20;
    return JSON.stringify(this.mergeLogs.slice(-limit), null, 2);
  }
  ...
  protected override async processToolCalls(content: ContentBlock[]): Promise<ToolResultBlock[]> {
    ...
    for (const block of content) {
      ...
      if (block.name === "spawn_worker") {
        ...
        result = { type: "tool_result", tool_use_id: block.id, content: output };
      } else if (block.name === "optimistic_merge") {
        const output = this.optimisticMerge(block.input as Record<string, unknown>);
        result = { type: "tool_result", tool_use_id: block.id, content: output };
      } else if (block.name === "list_fix_tasks") {
        const output = this.listFixTasks(block.input as Record<string, unknown>);
        result = { type: "tool_result", tool_use_id: block.id, content: output };
      } else if (block.name === "read_merge_log") {
        const output = this.readMergeLog(block.input as Record<string, unknown>);
        result = { type: "tool_result", tool_use_id: block.id, content: output };
  ...
  private getWorkerTools(): ToolDefinition[] {
    return [
      ...
      {
        name: "submit_handoff",
        description: "Submit structured handoff with worker/base diff.",
        input_schema: {
          type: "object",
          properties: {
            task_id: { type: "string" },
  ...
  getTools(): ToolDefinition[] {
    return [
      ...
      },
      {
        ...
        description: "File-based 3-way merge. On conflict create fix-forward task; do not revert.",
        input_schema: {
          type: "object",
          properties: {
            handoff_id: { type: "string" },
  ...
  getState(): AgentState {
    return {
      ...
      mergeLog: this.mergeLogs,
      fixForwardTasks: this.fixTasks,
    } as AgentState;
  }
}
